...
const f = new __WEBPACK_IMPORTED_MODULE_2__flock3d__["a" /* default */](100);
f.populateFlock();
...
class Flocky {
  ...
  populateFlock() {
    ...
    for (let i = 0; i < this.numBirds; i++) {
      ...
      const pos = this.randomPos();
      const vel = this.randomVel();
      this.birds.push(new __WEBPACK_IMPORTED_MODULE_0__bird3d__["a" /* default */](this, {key: i, pos: pos, vel: vel, radius: 4, color: 'black'}));
    }
    // this.birds.push(new Bird(this, {key: -5, pos: this.randomPos(), vel: this.randomVel(), radius: 4, color: 'yellow'}));
  }
  ...
  randomPos() {
    const xPos = Math.floor(Math.random()*1600);
    const yPos = Math.floor(Math.random()*900);
    const zPos = Math.floor(Math.random()*700);
...
class Birdy {
  ...
  draw(ctx) {
    ...
    const rad = (700 - depth)/700 * 5;
    const colorVal = rad * 32;
